# https: // github.com/switchablenorms/CelebAMask-HQ/blob/master/face_parsing/unet.py
    ...
    def __init__(
        ...
        if is_batchnorm:
            self.cb_unit = nn.Sequential(
                conv_mod, nn.BatchNorm2d(int(n_filters)))
        else:
...
def interp(input, output_size, mode="bilinear"):
    ...
    # normalize to [-1, 1]
    h = torch.arange(0, oh, dtype=torch.float,
                     device='cuda' if input.is_cuda else 'cpu') / (oh - 1) * 2 - 1
    w = torch.arange(0, ow, dtype=torch.float,
                     device='cuda' if input.is_cuda else 'cpu') / (ow - 1) * 2 - 1

    grid = torch.zeros(oh, ow, 2, dtype=torch.float,
